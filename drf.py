...
for j in range(3):      
    for i in range(0,iters[j]):
        clf_1 = RandomForestClassifier(n_estimators=10, max_depth=15)
        ...
        clf_2 = ExtraTreesClassifier(n_estimators=10, max_depth=15)
...
#print(np.average(cross_val_score(clf_1, new_train_features, np.ravel(y_train), cv = 10)))
#print(np.average(cross_val_score(clf_2, new_train_features, np.ravel(y_train), cv = 4)))
#print(np.average(cross_val_score(clf_3, new_train_features, np.ravel(y_train), cv = 4)))



drf_pred.tofile('test sub.csv',sep=',',format='%10.5f')